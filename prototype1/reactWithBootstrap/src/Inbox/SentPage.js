...
export default class SentPage extends Component {
  ...
  handleClick(e) {
    let url = config.settings.serverPath + "/api/inbox?type=sent&search=" + this.state.search;
    fetch(url, {
      method: 'GET',
      headers: {
        Accept: 'application/json',
  ...
  render() {
    ...
    if (this.state.data !== null) {
      EmailButtons = this.state.data.map((item, index) => {
        return (<EmailButton key={item.id} id={index} item={item} redirect={this.redirect} type="sent" />);
      })
    }
